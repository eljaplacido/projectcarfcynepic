"""C3 — C2 with a non-linear effect surface, and the reason it exists.

C2 answered SE1, SE5, SE6 and SE7 and produced one reading that cannot be
trusted: a cheap LINEAR causal model beat a gradient-boosted one on decision
regret. C2 generates individual effects as `segment_ate + 0.05 * x4`, so the
linear learner was near-correctly-specified and the boosted one was not. The
comparison measured specification match against a known generator.

C3 changes exactly one thing: the effect carries an interaction (`x4 * x5`) and
a threshold (`x4 > 0.6`), so **no linear model can be correctly specified**.
Everything else -- the segment design, the confounding, the treatment cost, the
S1 policy, every validator -- is C2's, unchanged, so the two corpora are
comparable cell for cell and any difference is attributable to the effect
surface.

If the cheap model still wins here, the reading generalises and is a real
finding about estimator families. If it loses, the C2 result was about
specification and must be restated.

Original C2 documentation follows, and applies except where noted above.
...
#: Expectation of C3's threshold term, subtracted to keep it mean-zero.
#: 0.16 * P(Z > 0.6) = 0.16 * 0.2743.
THRESHOLD_MEAN = 0.16 * 0.2743
...
def build(n_items: int, seed: int = SEED) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for i in range(n_items):
        ...
        # y1: the segment effect, plus individual variation that is a NON-LINEAR
        # function of x[4] and x[5]. This is the only substantive difference
        # from C2 and it is the whole point of C3.
        #
        # C2 used `seg.ate + 0.05 * x[4]` -- linear. The escalation work found a
        # cheap LINEAR causal model beating a gradient-boosted one there, and
        # that result is not interpretable: a linear learner is near-correctly-
        # specified against a linear generator, so the comparison measured
        # specification match rather than the merits of the estimator families.
        #
        # Here the effect carries an interaction and a threshold, so no linear
        # model can be correctly specified and the boosted one has something to
        # find. Individual effects stay independent of the risk score, because
        # x[4] and x[5] enter nothing else -- the decorrelation C2's validator
        # checks is preserved exactly.
        # Both terms are CENTRED so the segment's declared ATE stays true. The
        # non-linearity must add variation, not a shift: without centring the
        # threshold raised every segment's realised mean above its declared one
        # and `V1.segment_ates_recover` failed -- correctly, since the manifest
        # would then describe a corpus that was not generated.
        #
        # The interaction of two independent standard normals already has mean
        # zero. The threshold does not: P(Z > 0.6) = 1 - 0.7257 = 0.2743, so its
        # expectation is 0.16 * 0.2743 = 0.0439, subtracted below.
        interaction = 0.14 * x[4] * x[5]
        threshold = (0.16 if x[4] > 0.6 else 0.0) - THRESHOLD_MEAN
        effect = float(seg.ate + interaction + threshold)
    ...
    manifest = {
        "corpus_id": "C3",
        "title": "Shared decision corpus, NON-LINEAR effect surface",
        "seed": seed,
        "n_items": n_items,
...
def main() -> None:
    ...
    corpus_path = args.out_dir / "c3_corpus.jsonl"
    with open(corpus_path, "w", encoding="utf-8") as fh:
        for r in records:
            fh.write(json.dumps(r) + "\n")

    manifest_path = args.out_dir / "c3_manifest.json"
    ...
    logger.info("C3 built: %d items -> %s", len(records), corpus_path)
