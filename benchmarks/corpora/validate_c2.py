...
CORPUS = Path(__file__).parent / "c2_corpus.jsonl"
MANIFEST = Path(__file__).parent / "c2_manifest.json"
...
def _load() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if not CORPUS.exists():
        raise CheckFailed(f"{CORPUS} missing - run build_c2.py first")
...
def v1_segment_ates_recover(records: list[dict[str, Any]], manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, ate in declared.items():
        ...
        # Tolerance scales with sample size: these are means of a few dozen
        # draws around the declared value, not the value itself.
        tol = 0.02 + 2.5 / max(len(ites), 1) ** 0.5 * 0.06
        if abs(got - ate) > tol:
...
def v1_deterministic(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from benchmarks.corpora.build_c2 import build
...
def main() -> int:
    ...
    logger.info("C2: %d items, seed %s", len(records), manifest["seed"])
    ...
    out = Path(__file__).parent / "c2_validation.json"
    ...
    if skipped:
        ...
        logger.info("V1/V2 PASS. C2 is usable. -> %s", out)
    return 0
